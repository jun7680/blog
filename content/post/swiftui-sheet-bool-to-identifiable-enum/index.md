...
홈 화면에 모달이 세 개 있다고 치자. 초안에 가장 흔한 모양이다.
...
지나고 보니 Bool 두 개 정도까지는 그냥 써도 큰 문제는 없는데 세 개째가 들어오는 순간엔 거의 항상 enum으로 갈아엎는 게 답이었고 결국 "지금 띄울 게 뭐냐"라는 질문이 본질적으로 하나의 값이라는 사실을 코드에 반영해주는 일이었다.
