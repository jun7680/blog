...
문제: **신규 `@Model` 추가 시 누락되면 영구 보존됨**. 새 도메인 생겼는데 deletion 호출을 까먹으면, 사용자가 로그아웃해도 그 데이터가 안 지워짐. 다른 계정으로 로그인하면 이전 계정 데이터가 보일 수 있음. **누락이 안전을 깬다** → 위험.
...
단점: **Entity마다 conformance 추가가 필요하고, 보존 의사 표현이 47개 파일에 흩어짐**. 정책 변경 시 한눈에 파악이 안 됨. 마이그 같은 대규모 작업에선 변경 추적이 빡세다.
